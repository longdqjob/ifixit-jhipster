...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/item-type'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: ItemType) {
        return item.id;
    }
    registerChangeInItemTypes() {
        this.eventSubscriber = this.eventManager.subscribe('itemTypeListModification', (response) => this.loadAll());
    }
